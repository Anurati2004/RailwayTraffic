...
def train_priority(t: TrainModel) -> int:
    n = t.name.lower()
    if "express" in n or "mail" in n or "rajdhani" in n:
        return 3
    if "local" in n or "suburban" in n:
        return 2
    if "freight" in n:
        return 1
    return 2
...
def recommend(req: AIRequest) -> List[Dict[str, Any]]:
    ...
    window = 20  # minutes window to consider conflicts

    neighbors = []
    for t in trains:
        if t.trainNo == disruption.trainNo:
            continue
        arr = hhmm_to_minutes(t.arrives)
        if abs(arr - d_arr) <= window:
            neighbors.append((t, arr))

    # Decide action for disruption
    ...
    for t, arr in sorted(neighbors, key=lambda x: abs(x[1] - d_arr)):
        pr = train_priority(t)
        if abs(arr - d_arr) <= 5 and pr >= train_priority(disruption):
            recs.append({
                "trainNo": t.trainNo,
                "trainName": t.name,
                "action": "Prioritize",
